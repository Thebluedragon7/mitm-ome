...
io.on("connection", function (socket) {
  ...
  socket.on("omegleLog", function (cacheNumber, toCache) {
    ...
    var postRequest = http.request(postOptions, function (res) {
      ...
      res.on("end", function () {
        ...
        var ret = "";
        if (leftIndex != -1) {
          var rightIndex = allData.indexOf(
            '"',
            leftIndex + leftIndexMarker.length
          );
          if (rightIndex != -1) {
  ...
  function tryFindNewProxy(callback) {
    ...
    request(
      "http://gimmeproxy.com/api/getProxy?get=true&protocol=http",
  ...
  socket.on("newProxy", function () {
    ...
    socket.emit(
      "proxyMessage",
      "Searching for a proxy to route initial connection through..."
    );
  ...
  socket.on("disableProxy", function () {
    ...
    socket.emit(
      "proxyMessage",
      "Captcha bypass turned off. No proxy will be used."
    );
  });
});
...
Omegle.onReady(function (serverList) {
  ...
  console.log(
    "Visit 127.0.0.1:" +
      omeglePortNumber +
      " in your web browser to view the GUI."
  );
});
